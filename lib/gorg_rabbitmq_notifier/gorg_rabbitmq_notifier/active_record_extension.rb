...
module GorgRabbitmqNotifier::ActiveRecordExtension
  ...
  included do
    ...
    def self.rabbitmq_id(method_name=nil,&block)
      @rabbitmq_id_proc=( block_given? ? block : Proc.new {|o| o.send(method_name) } )
    end
    ...
    def send_rabbitmq_notification(action,changes={})
      message=GorgRabbitmqNotifier::NotificationMessage.new(
    ...
    protected
      ...
      def process_association(action,from_object, to_object)
        if to_object.respond_to? :rabbitmq_id
          ...
          association=from_serializer._reflections.find{|s| s.options[:serializer] == to_serializer}
          case association.class.to_s
          when "ActiveModel::Serializer::HasManyReflection"
            from_object.send_rabbitmq_notification(:updated,changes={association.name => {action =>[to_object.rabbitmq_id]}})
