...
RSpec.describe LdapDaemon, type: :service do
  ...
  it "request a message sending to the message sender for account update" do
    ...
    expect(message_sender).to have_received.send_message({account: {id_soce: "1234"}}, 'request.ldapd.update')
  end
  ...
  it "request a message sending to the message sender for group update" do
    ...
    expect(message_sender).to have_received.send_message({group: {short_name: "test_group"}}, 'request.ldapd.update')
  end
  ...
  it "request a message sending to the message sender for role update" do
    ...
    expect(message_sender).to have_received.send_message({role: {name: "test_role", application: "test_app"}}, 'request.ldapd.update')
  end
end
