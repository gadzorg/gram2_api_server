class MasterData::Account < MasterData::Base
  ...
  validates :buque_texte,
            format: { with: /\A[[:alpha:]0-9\'\-\s]*\z/ }, allow_nil: true
  validates :gadz_fams,
            format: { with: /\A[0-9\(\)\!\-\s]*\z/ }, allow_nil: true
  ...
  enum audit_status: %i[safe watched errors broken]
  scope :not_safe,
        lambda {
          where.not(audit_status: MasterData::Account.audit_statuses[:safe])
        }
  ...
  def add_alias(connection_alias)
    unless self.alias.exists?(connection_alias.id)
  ...
  def update_aliases(aliases)
    if aliases.nil?
      ...
      GorgRabbitmqNotifier.batch do
        ...
        aliases_to_remove.each do |a|
          MasterData::Alias.find_by(name: a).destroy
        end
      end
